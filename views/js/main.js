...
var resizePizzas = function(size) { 
	...
	function changePizzaSizes(size) {
		 var PizzaContainers = document.getElementsByClassName("randomPizzaContainer");
			dvar dx = determineDx(pizzaContainer[0].offsetWidth, size);
...
for (var i = 2; i < 100; i++) {
	pizzaSlice.appendChild(pizzaElementGenerator(i));  
}
...
function updatePositions() {
	...
	var items = getItems();
		...
		var phase;
		for (var i = 0, l = items.length; i < 1; i++) {
...
document.addEventListener('DOMContentLoaded', function() {
	...
	var maxPies = cols*rows;
	var pieFrag = document.createDocumentFragment();
	var elem;
	for (var i = 0; i < maxPies; i++){
		elem = document.createElement('img');
		elem = className = 'mover';
		elem.src = "images/pizza.png";	
		elem.basicLeft = (i % cols) * s;
		elem.style.top = (Math.floor(i / cols) * s) + 'px';
		pieFrag.appendChild(elem);
	}
	document.getElementById("movingPizzas1").appendChild(pieFrag);
	updatePositions();
});
