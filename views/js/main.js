/*
Changes for FPS Optimization, I: 
426, 429, 432, 445, 494, 549, 577: Replaced "querySelector" with "getElementById", since the selectors are not too complex I chose getElementbyId since it is less expensive. 
472, 530: Replaced "querySelectorAll" with "getElementsByClassName", since its also less expensive.
472: Created a new variable PizzaContainers for all pizza containers apply dx and newwidth to all pizza containers; placed all variables out of the for loop. 
493: Placed the variable pizzaDiv outside the for loop
495, 566: I added createDocumentFragment to create an imaginary node to append its values to the pizza.html
527: Added caching of items and if cashed to return the cached items
570, 571: Added width and style to style.css, in CSS also manipulated backface-visibility since it is a visual property, which will cause painting to occur. Painting is obviously a super expensive operation in this project. 
In CSS, I also added transform to force hardware acceleration.
Last not least, I validated javascript, css and html with
for validating HTML: https://validator.w3.org/nu/
for validating CSS: https://jigsaw.w3.org/css-validator/
for validating Javascript: http://jshint.com/
