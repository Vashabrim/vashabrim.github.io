...
function AppendPizza() {
    ...
    for (var i = 2; i < 100; i++) {
        //pizzasDiv.appendChild(pizzaElementGenerator(i)); change of plans
    ...
    // Generated number of pizza images depending on screen size
    document.addEventListener('DOMContentLoaded', function () {
        var vWid = window.screen.width;
        var vHt = window.screen.height;
        var s = 256;
        var cols = Math.round(vWid / 200) + 2;
        var rows = Math.round(vHt / 200);
        ...
        for (var i = 0; i < maxPizzas; i++) {
            ...
            myFrag.appendChild(elem);
        }
        document.getElementById("movingPizzas1").appendChild(myFrag);
        updatePositions();
    });
}